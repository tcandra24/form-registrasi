                                    ...
                                    @foreach ($jobs as $key => $job)
                                        <tr>
                                            <td class="border-bottom-0 pb-0"">
                                                <h6 class="fw-semibold mb-0">{{ $jobs->firstItem() + $key }}</h6>
                                            </td>
                                            <td class="border-bottom-0 pb-0"">
                                            ...
                                            <td class="border-bottom-0 pb-0"">
                                                <div class="d-flex align-items-center gap-2">
                                                    @can('master.jobs.edit')
                                                        <a href="/jobs/{{ $job->id }}/edit" class="btn btn-success m-1">
                                                            <i class="ti ti-pencil"></i>
                                                        </a>
                                                    @endcan
                                                    @can('master.jobs.delete')
                                                        <button class="btn btn-danger m-1 btn-delete"
                                                            data-id="{{ $job->id }}" data-name="{{ $job->name }}"><i
                                                                class="ti ti-trash"></i></button>

                                                        <form id="form-delete-job-{{ $job->id }}" method="POST"
                                                            action=" {{ url('/jobs/' . $job->id) }}">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
                                                    @endcan
